...
ggplot(data = df1, aes(x=부서, y=매출)) +
  ...
  labs(title = "팀별 영업실적", x="부서", y="매출(억원)")
...
library(ggplot2)
ggplot(data = df1, aes(x=부서, y=매출)) +
  geom_bar(stat = "identity", fill = "#F564E3", color = "black") +
  labs(title = "팀별 영업실적", x="부서", y="매출(억원)")

# 막대마다 사용자가 지정한 색상으로 변경
ggplot(data = df1, aes(x=부서, y=매출))+
  geom_bar(stat = "identity", fill=c("tomato", "gold", "purple"), color="black")+
  labs(title = "팀 별 영업실적", x="부서명", y="매출(억원)")

# gdRdriver, RColorBrewer 색상팔레트를 제공하는 두 가지 패키지
...
# 부서별 분기별 막대그래프 작성
# 조건 1 > 3분기 실적을 추가
# 조건 2 > gdRdriver 패키지의 팔레트에 있는 색상을 막대에 설정
# 조건 3 > 그래프 제목: 부서별 분기별 영업 실적, x축: 부서명, y축: 영업매출(억원) 
dept = c("영업1팀", "영업2팀", "영업3팀")
sales = c(8, 7, 6)
...
ggplot(df, aes(x=부서, y=매출, fill = 분기)) +
  geom_bar(stat="identity", fill = heat.colors(9)) +
  labs(title="부서별 분기별 영업 실적", x="부서명", y="영업매출(억원)")

# 분기 별로 사용자 지정색 설정
ggplot(df, aes(x=부서, y=매출, fill = 분기)) +
  geom_bar(stat="identity") +
  scale_fill_manual(values = c("pink", "red", "purple")) +
  labs(title="부서별 분기별 영업 실적", x="부서명", y="영업매출(억원)")
