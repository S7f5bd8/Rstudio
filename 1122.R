...
Y = c(10, 20, NULL, 40, 50, NA, 70, 80, 90, 100, 110)
...
ggpairs(data=iris, columns = 1:4, aes(color=Species, alpha=0.5), 
...
# df에서 W의 빈문자열이 아닌 값과 X, Z의 유한한 값인 행과 특성(W, X열) 추출
...
df2 = data.frame(ID=1:3, 성명 = c('장발장','팡틴','자베르'))
...
# left outer join (왼쪽 외부 조인: 왼쪽의 데이터 프레임이 손실되지 않는다.)
left_join(df1, df2, by = 'ID')


# right outer join (왼쪽 외부 조인: 오른쪽의 데이터 프레임이 손실되지 않는다.)
