...
# 표본추출방법: 복원 추출, 비복원 추출
# 복원추출 ex) 10개의 값 중에서 추출된 값을 다시 사용하여 추출
# 비복원추출 ex) 10개의 값 중에서 이미 추출된 값을 제외하고  추출
...
# 동전던지기 반복문
for (variavle in vector)
  
# 동전 던지기 반복
for(x in 1:iteration) {
  # 동전 던지기(추출 개수 1개이므로 복원 또는 비복원 추출의 결과가 같다.)
