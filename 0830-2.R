...
#벡터에 있는 요소의 값들 중에 하나라도 비교연산 결과에 true가 있으면 결과는 true로 표시
any(x<=10)
#벡터에 있는 요소의 값들이 비교 연산 결과에 true가 되어야 결과는 true
all(x<=10)
#벡터에서 20이상인 요소를 반환
x[x>=20]
...
#요인(factor)
#벡터에 저장된 동일한 값들을 요인으로 찾아내서 대표값들을 분류해서 반환 + 원래 벡터값들을 함께 반환
